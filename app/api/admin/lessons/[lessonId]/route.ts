...
export async function GET(
  ...
  try {
    ...
    console.error("[ADMIN_LESSON_GLOBAL_GET]", error);
...
export async function PATCH(
  ...
  try {
    ...
    await logActivity({
      ...
      targetType: "lesson",
      targetId: lessonId,
      details: `Updated lesson: "${updated.title}" (${existing.module?.course?.title})`,
    });

    return NextResponse.json(updated);
  } catch (error: any) {
    console.error("[ADMIN_LESSON_GLOBAL_PATCH]", error);
    return NextResponse.json({ error: error?.message || "Internal Error" }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    console.error("[ADMIN_LESSON_GLOBAL_DELETE]", error);
    return NextResponse.json({ error: error?.message || "Internal Error" }, { status: 500 });
  }
}
